...
require_once('lib/generator/JsDocSerializer.php');
...
if ($clean) {
  ...
  if ($kwargs['outfile']) {
    ...
    if (isset($kwargs['serialize']['jsdoc'])) {
      JsDocSerializer::clean('cache', 'hdr.js', $kwargs['outfile']);
    }
  }
  else {
    if (isset($kwargs['serialize']['json'])) {
    ...
    if (isset($kwargs['serialize']['jsdoc'])) {
      JsDocSerializer::clean('cache', 'hdr.js');
    }
  }
}
...
if ($kwargs['outfile']) {
  ...
  if (isset($kwargs['serialize']['jsdoc'])) {
    $serializers['jsdoc'] = new JsDocSerializer('cache', 'hdr.js', $kwargs['outfile']);
  }
}
else {
  if (isset($kwargs['serialize']['json'])) {
  ...
  if (isset($kwargs['serialize']['jsdoc'])) {
    $serializers['jsdoc'] = new JsDocSerializer('cache', 'hdr.js');
  }
}
...
//  if($id != "dojo" && $id != "doh.robot") continue; // namespace test
